...
rooms_collection = chat_database.get_collection('rooms')
room_members_collection = chat_database.get_collection('room_members')
messages_collection = chat_database.get_collection('messages')
...
def save_room(room_name, created_by):
    room_id = rooms_collection.insert_one({'room_name': room_name, 'created_by': created_by,
                                           'time_added': datetime.now()}).inserted_id
...
def add_room_member(room_id, room_name, username, added_by, is_room_admin=False):
    room_members_collection.insert_one({'_id': {'room_id': ObjectId(room_id), 'username': username},
                                        ...
                                        'time_added': datetime.now(), 'is_room_admin': is_room_admin})


def add_room_members(room_id, room_name, usernames, added_by):
    room_members_collection.insert_many(
        [{'_id': {'room_id': ObjectId(room_id), 'username': username}, 'room_name': room_name, 'added_by': added_by,
          'time_added': datetime.now(), 'is_room_admin': False} for username in usernames])
...
def save_message(room_id, text, sender):
    messages_collection.insert_one({'room_id': room_id, 'text': text, 'sender': sender, 'send_time': datetime.now()})
...
def get_messages(room_id, page=0):
    ...
    for message in messages:
        message['send_time'] = message['send_time'].strftime("%d-%b-%Y, %H:%M:%S")
    return messages[::-1]
